...
/**
 * Manager Objektum (Singleton), tartalmazza a pálya mezõit és a kígyót A kígyón
 * mûveletet õ tud végezni
 *
 */
public class Manager {
	...
	private static final int MIN_POISON = 3;

	/**
	 * Poison Drop Rate [s]
	 */
	private static final int POISON_DROP_RATE = 10;
	...
	private boolean checkNotEmpty(Field randomField) {
		...
		boolean notEmpty = fields.contains(randomField)
				|| poisonContainers != null && poisonContainers.contains(randomField) || randomField == foodContainer;
		return notEmpty;
	}

	public void dropPoison() {
		if (poisonContainers.size() < maxPoison) {
			Field randomField = getEmptyField();

			Poison poison = new Poison();
			randomField.addCollectable(poison);
